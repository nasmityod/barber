...
export function BookingApp({ slug, initialCatalog }: { slug: string; initialCatalog: BookingCatalog | null }) {
  ...
  if (loadingError) return <div className="booking-site"><BookingHeader name="787 Barber Studio" logoUrl="" /><main className="success-card"><span className="eyebrow">Reserva segura</span><h1>No encontramos esta barbería.</h1><p>{loadingError}</p></main></div>;
  if (!catalog || !service || !professional) return <div className="booking-loading"><BrandMark /><strong>Cargando agenda segura…</strong></div>;
    ...
    <div className="booking-site book-live" style={publicStyle}>
      ...
      <main className="book-stage">
        <aside className="book-ticket" aria-label="Resumen de tu cita">
          <BrandMark className="book-ticket-mark" />
...
function BookingBrand({ name, logoUrl, caption }: { name: string; logoUrl: string; caption: string }) {
  return <div className="booking-brand">
    {}
    {logoUrl ? <img className="custom-logo" src={logoUrl} alt="" /> : <BrandMark />}
    <div><strong>{name.toUpperCase()}</strong><small>{caption}</small></div>
  </div>;
}
function BookingHeader({ name, logoUrl }: { name: string; logoUrl: string }) { return <header className="booking-header"><BookingBrand name={name} logoUrl={logoUrl} caption="Reservas online" /><div className="secure-booking"><ShieldCheck /><span>Conexión y reserva protegidas</span></div></header>; }
...
function BookingFooter({ name }: { name: string }) { return <footer className="booking-footer"><BookingBrand name={name} logoUrl="" caption="787 · Barber Studio" /><p>Privacidad por diseño · Datos mínimos</p><span className="secure-booking"><ShieldCheck size={13} /> Reserva protegida</span></footer>; }
