  ...
  return (
    <div className="admin-shell">
      ...
      <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
        <div className="brand"><BrandMark alt="787 Barber Studio" /><span className="brand-word"><strong>Barber Studio</strong><small>Panel operativo</small></span></div>
      ...
      <main className="admin-main">
        <header className="topbar">
          ...
          <span className="topbar-brand"><BrandMark alt="787 Barber Studio" /></span>
          <button type="button" className="topbar-search" onClick={() => setCommandOpen(true)} aria-label="Buscar en todo el panel"><Search size={17} /><span>Buscar cliente, cita, servicio o sección…</span><kbd>Ctrl K</kbd></button>
...
function Dashboard({ appointments, timezone, slug, catalog }: { appointments: Appointment[]; timezone: string; slug: string; catalog: Catalog|null }) {
  ...
  return <>
    <div className="rule-787" aria-hidden="true" />
    <Onboarding appointments={appointments} catalog={catalog} slug={slug} />
    {/* Un solo indicador manda: lo cobrado hoy. El resto acompaña. */}
    <section className="shop-strip today-plate" aria-label="Resumen de hoy">
      <div className="today-lead">
        <span>Piso abierto</span>
        <p className="today-label">Cobrado hoy</p>
        <strong className="today-amount">{moneyCents(todayRevenue, currency)}</strong>
        <p className="today-note">{ticket ? `Ticket medio ${moneyCents(Math.round(todayRevenue / Math.max(1, ticket)), currency)} · ${ticket} ${ticket === 1 ? "cobro" : "cobros"}` : "Todavía sin cobros registrados hoy"}</p>
      </div>
      <dl className="today-stats">
        <div><dt>Citas de hoy</dt><dd>{String(todayAppointments.length).padStart(2, "0")}</dd></div>
        <div><dt>Por atender</dt><dd>{String(pendingToday.length).padStart(2, "0")}</dd></div>
        <div><dt>Próximas</dt><dd>{String(future.length).padStart(2, "0")}</dd></div>
      </dl>
      <div className="shop-actions">
        <Link className="primary" href="/turno"><ListOrdered size={15} /> Turno del día</Link>
        <Link className="secondary" href="/caja">Caja</Link>
        <Link className="secondary" href="/reportes">Reportes</Link>
    ...
    <section className="dashboard-grid">
      ...
      <div className="panel"><PanelTitle title="Próximos clientes" subtitle={`${future.length} citas por atender`} />
...
function Onboarding({ appointments, catalog, slug }: { appointments: Appointment[]; catalog: Catalog|null; slug: string }) {
  ...
  return <section className="panel onboarding-panel">
    ...
    <p style={{ marginTop: 14, fontSize: 13, color: "var(--text-secondary)" }}>Tu página pública ya vive en /reservar/{slug}</p>
  </section>;
}
...
function PlanMeter({ appointments, timezone }: { appointments: Appointment[]; timezone: string }) {
  ...
  return <div className="sidebar-footer">
    ...
    {limit > 0 && <div className="plan-progress"><i style={{ width: `${percent}%`, background: percent >= 90 ? "var(--danger)" : undefined }} /></div>}
...
function Clients({onClientUpdated}:{onClientUpdated:(client:ClientRecord)=>void}){
  ...
  return <>
    <section className="metric-grid three"><Metric icon={<UsersRound/>} tone="gold" label="Total clientes" value={String(clients.length)} trend="Registros persistentes"/><Metric icon={<Star/>} tone="ok" label="Recurrentes" value={clients.length?`${Math.round(recurrent/clients.length*100)}%`:"0%"} trend={`${recurrent} con más de una cita`}/><Metric icon={<TrendingUp/>} tone="info" label="Ticket promedio" value={`$${(revenue/Math.max(1,completed)/100).toFixed(2)}`} trend="Sobre citas completadas"/></section>
...
function Services({initialServices,onCatalogChanged}:{initialServices:Service[];onCatalogChanged:(services:Service[])=>void}){
  ...
  return <>
    <section className="metric-grid three"><Metric icon={<Scissors/>} tone="gold" label="Servicios" value={String(services.length)} trend="Catálogo persistente"/><Metric icon={<CheckCircle2/>} tone="ok" label="Activos" value={String(activeCount)} trend="Visibles al reservar"/><Metric icon={<Clock3/>} tone="info" label="Duración promedio" value={`${services.length?Math.round(services.reduce((sum,item)=>sum+item.durationMinutes,0)/services.length):0} min`} trend="Configurada por servicio"/></section>
...
function Team({initialProfessionals,services,onCatalogChanged}:{initialProfessionals:Professional[];services:Service[];onCatalogChanged:(professionals:Professional[],changed?:Professional)=>void}){
  ...
  return <>
    <section className="metric-grid three"><Metric icon={<UserRound/>} tone="gold" label="Profesionales" value={String(professionals.length)} trend="Perfiles persistentes"/><Metric icon={<CheckCircle2/>} tone="ok" label="Activos" value={String(activeCount)} trend="Disponibles para reservar"/><Metric icon={<TrendingUp/>} tone="info" label="Servicios completados" value={String(completedCount)} trend="Historial real del equipo"/></section>
