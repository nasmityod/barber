...
export default async function PaymentPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <main className="public-payment"><div className="public-payment-card"><span className="eyebrow">{request.businessName}</span><h1>{expired ? "Solicitud no disponible" : "Confirma tu reserva"}</h1>{request.clientName && <p>Hola, {request.clientName}. Para confirmar tu espacio registra el depósito solicitado.</p>}{expired ? <div className="payment-expired">Esta solicitud está {request.status === "pending" ? "vencida" : request.status === "paid" ? "confirmada" : "cerrada"}.</div> : <><div className="payment-amount"><span>Depósito solicitado</span><strong>{money(request.depositCents)}</strong><small>Total de la reserva: {money(request.amountCents)}</small></div>{request.checkoutUrl ? <a className="primary payment-link" href={request.checkoutUrl} target="_blank" rel="noreferrer">Pagar con {providerLabel(request.provider)}</a> : <div className="payment-instructions"><strong>Depósito manual</strong><p>Contacta a la barbería para recibir los datos de transferencia o pago móvil y luego informa la referencia.</p><PublicPaymentForm token={token} /></div>}{request.status === "submitted" && <p className="payment-pending">Comprobante enviado; queda pendiente de verificación.</p>}<small className="payment-security">No compartas este enlace. Caduca automáticamente en 7 días.</small></>}</div></main>;
}
